...
export default function PlayPage() {
  ...
  useEffect(() => {
    ...
    s.on(WS_EVENTS.ERROR, (e: any) => {
      toast.push({ tone: "danger", title: "Error", description: e?.code });
    });
  ...
  }, [token, setRoom, setRemaining, setReveal, toast]);
    ...
    try {
      ...
      if (locations.length < rounds) {
        toast.push({
          ...
          description: `Got ${locations.length}/${rounds} panoramas — backend will fill the rest.`,
        });
      }
      ...
      toast.push({
        tone: "danger",
        title: "Could not pick panoramas",
  ...
  }, [token, selectedPackId, packs, room?.config.rounds, toast]);
    ...
    return (
      <div className="max-w-4xl mx-auto py-10 flex flex-col gap-6 animate-fade-in">
        ...
        {isHost && (
          <Card>
            <CardHeader>
              <CardTitle>Map pack</CardTitle>
              <Badge tone="violet">host pick</Badge>
            </CardHeader>
            <CardBody>
        ...
        {!isHost && (
          <div className="text-center text-sm text-ink-muted">
            ...
            <span className="text-brand-cyan font-medium">
              {packs.find((p) => p.id === room.config.packId)?.name ?? "World"}
            </span>
          </div>
        )}
        ...
        <div className="flex gap-3 justify-center">
          {isHost ? (
            ...
            >
              {preparing
                ? "Picking panoramas…"
                : `Start game (${room.config.rounds} rounds · ${room.config.roundSeconds}s · ${packs.find((p) => p.id === selectedPackId)?.emoji ?? ""} ${packs.find((p) => p.id === selectedPackId)?.name ?? "World"})`}
            </Button>
          ) : (
  ...
  return (
    ...
    >
      ...
      {isGuessing && (
        ...
        >
          {guess ? "GUESS" : "drop a pin on the map"}
        </button>
      )}
    ...
    <div className="max-w-xl mx-auto py-16 animate-fade-in">
      <Card glow>
        <CardHeader>
          ...
          <Badge tone="gold">final standings</Badge>
        </CardHeader>
        <CardBody className="flex flex-col gap-5">
          ...
          <div className="flex gap-3 justify-end">
            ...
            <Link href="/leaderboard">
              <Button variant="secondary">Global ranking</Button>
            </Link>
